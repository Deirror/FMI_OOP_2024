...
void StringVector::push_back(String && other)
{
	if (_size == _capacity)
		resize(roundToPowerOfTwo(_size + 1));

	_strings[_size++] = std::move(other);
}

void StringVector::insert(String && other, size_t index)
{
	if (index > _size)
		throw std::exception("Out of range");
